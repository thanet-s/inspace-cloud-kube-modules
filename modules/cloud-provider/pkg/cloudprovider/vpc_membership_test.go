...
func TestCanonicalConfiguredVPCVMUUIDs(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			members, err := canonicalConfiguredVPCVMUUIDs(location, test.network)
			if test.wantError {
			...
			for _, member := range test.wantMembers {
				if source, present := members[member]; !present || source != member {
					t.Fatalf("canonical membership %s = %q, %t; want exact source identity", member, source, present)
				}
			}
		})
	}
