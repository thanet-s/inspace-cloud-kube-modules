...
var (
	Families = []Family{
		{Name: "compute", MemoryGiBPerVCPU: 1},
		{Name: "general", MemoryGiBPerVCPU: 2},
		{Name: "memory", MemoryGiBPerVCPU: 4},
	}
...
// New returns the complete, finite 24-variant InSpace catalog. Prices are
...
func New(opts Options) ([]*cloudprovider.InstanceType, error) {
	...
	result := make([]*cloudprovider.InstanceType, 0, len(Families)*len(CoreCounts))
...
func relativePrice(cores, memoryGiB int, hostClass string) float64 {
	price := float64(cores) + float64(memoryGiB)/4
	if hostClass == inspacev1.HostClassAMDEPYC {
		price *= 1.1
	}
	return price
}
